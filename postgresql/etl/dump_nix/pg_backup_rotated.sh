...
function perform_backups()
{
	...
	do
		...
		then
			...
			if ! pg_dump $OPTIONS -Fp -h "$HOSTNAME" -U "$USERNAME" "$DATABASE" | gzip > $FINAL_BACKUP_DIR"$DATABASE".sql.gz.in_progress; then
		...
		then
			...
			if ! pg_dump $OPTIONS -Fc -h "$HOSTNAME" -U "$USERNAME" "$DATABASE" -f $FINAL_BACKUP_DIR"$DATABASE".custom.in_progress; then
