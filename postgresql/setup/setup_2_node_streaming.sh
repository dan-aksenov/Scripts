...
# Check connection
wget --spider https://download.postgresql.org/pub/repos/yum/$PGVER/redhat/rhel-7-x86_64/pgdg-redhat-repo-latest.noarch.rpm
if [ "$?" != 0 ]; then
  echo "Unable to connect to yum.postgresql.org. Check network of proxy settints."
fi

# Add Posgresql repo
yum -y install https://download.postgresql.org/pub/repos/yum/$PGVER/redhat/rhel-7-x86_64/pgdg-redhat-repo-latest.noarch.rpm
...
# Install some languages if needed
